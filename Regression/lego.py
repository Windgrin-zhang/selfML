...
def crossValidation(xArr, yArr, numVal = 10):
	...
	for i in range(numVal):																#交叉验证numVal次
		...
		wMat = ridgeTest(trainX, trainY)    											#获得30个不同lambda下的岭Regression Coefficients
		for k in range(30):																#遍历所有的岭Regression Coefficients
			matTestX = np.asmatrix(testX); matTrainX = np.asmatrix(trainX)						#测试集
			meanTrain = np.mean(matTrainX,0)											#测试集均值
			varTrain = np.var(matTrainX,0)												#测试集方差
			matTestX = (matTestX - meanTrain) / varTrain 								#测试集标准化
			yEst = matTestX * np.asmatrix(wMat[k,:]).T + np.mean(trainY)						#根据ws预测y值
			errorMat[i, k] = rssError(yEst.T.A, np.array(testY))							#统计误差
	meanErrors = np.mean(errorMat,0)													#计算每次交叉验证的平均误差
	minMean = float(min(meanErrors))													#找到最小误差
	bestWeights = wMat[np.nonzero(meanErrors == minMean)]								#找到最佳Regression Coefficients
...
def useStandRegres():
	...
	lgX1 = np.asmatrix(np.ones((data_num, features_num + 1)))
	lgX1[:, 1:5] = np.asmatrix(lgX)

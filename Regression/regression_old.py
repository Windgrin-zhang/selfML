...
def plotRegression():
	...
	"""
	xArr, yArr = loadDataSet('ex0.txt')									#加载数据集
	ws = standRegres(xArr, yArr)										#计算Regression Coefficients
	xMat = np.asmatrix(xArr)													#创建xMat矩阵
	yMat = np.asmatrix(yArr)													#创建yMat矩阵
...
def plotlwlrRegression():
	...
	axs0_title_text = axs[0].set_title(u'Locally Weighted Regression, k=1.0',fontsize=14)
	axs1_title_text = axs[1].set_title(u'Locally Weighted Regression, k=0.01',fontsize=14)
	axs2_title_text = axs[2].set_title(u'Locally Weighted Regression, k=0.003',fontsize=14)
